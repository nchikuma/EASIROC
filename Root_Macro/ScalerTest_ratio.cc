...
const char* PrintFileName_Scaler = "C:/Users/nchikuma/Documents/NaruhiroChikuma/EASIROC/data/figure/ScalerTest_ratio.eps";
...
void ScalerTest_ratio(){
  ...
  for(int i=0;i<nData;i++){
    ...
    xx[i] = inFreq;
    xe[i] = inFerr;
    yy[i] = mean_scaler/10000./xx[i];
    ye[i] = sigma_scaler/10000./xx[i];
  ...
  scalerGraph->GetXaxis()->SetTitleOffset(1.5);
  scalerGraph->GetYaxis()->SetTitle("Frequency ratio of scaler counts to input pulse");
  scalerGraph->GetYaxis()->SetRangeUser(0.96,1.04);
  ...
  f1_scaler->SetParameter(0,1.);
  f1_scaler->SetParameter(1,0.);
  ...
  TLegend *leg = new TLegend(0.70,0.75,0.88,0.85);
  leg->SetBorderSize(1);
  leg->SetFillColor(kWhite);
  //leg->SetFillStyle(3000);
  leg->AddEntry(scalerGraph,"Data","p");
  leg->AddEntry(f1_scaler,"Linear line (y=0)","l");
  leg->Draw("");
