...
class SpiCommandSender
	...
	puts "class SpiCommandSender initilalize" ##DEBUG
    ...
    def reconfigure
        @rbcp.write(status_register_address, 'M')
        @rbcp.write(status_register_address, 'I')
        @rbcp.write(status_register_address, 'W')
        begin
            @rbcp.write(status_register_address, 'A')
        rescue RBCPError => e
            raise e unless e.message == 'Timeout'
            return
        end
        raise SpiCommandSenderException.new('Reconfiguration failed')
    end
