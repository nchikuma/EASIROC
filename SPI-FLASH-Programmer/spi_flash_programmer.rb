...
class SpiFlashProgrammer
    def initialize(host: '192.168.10.16', port: 4660, rbcp_address: 0, mcs_filename: '', quiet_mode: false)

	puts "SpiFlashProgrammer initialize"  ## DEBUG
        if quiet_mode
        ...
        puts "start create_spi_command_sender(host, port, rbcp_address)" ##DEBUG
        create_spi_command_sender(host, port, rbcp_address)

        if !quiet_mode
            print_device_information
        end
        puts "start read_mcs(mcs_filename)" ##DEBUG
    ...
    def reconfigure
        puts 'Reconfiguring'
        begin
            @spi_command_sender.reconfigure
...
opt.on('--no-reconfigure', "Don't reconfigure at end of programming"){|v|
    OPTS[:dont_reconfigure] = v
}
opt.parse!(ARGV)
...
begin
    ...
    puts "finish spi_flash_programmer = SpiFlashProgrammer.new" ##DEBUG
    spi_flash_programmer.erase
    spi_flash_programmer.write
    spi_flash_programmer.verify
    unless OPTS[:dont_reconfigure]
        spi_flash_programmer.reconfigure
    end
rescue RBCPError => e
